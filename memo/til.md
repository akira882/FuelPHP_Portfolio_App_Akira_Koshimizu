...
## 反省点
- pullしてリモートからコードを取得してから、pushする
→コンフリクトが発生、解決しないとpushできない

# 更新休止理由
親族の不幸により、1月29日から5日間ほど更新を休止いたします。


# 次回のステップ案（FuelPHP学習）

## c1 「M（Model）」に挑戦！
今はコントローラーに直接データを書いていますが、本番は「データベース」からデータを取ってきます。
データベースから掲示板などのデータを取ってくる仕組みを作ります。

## c2 「共通レイアウト」を作ろう！
ページが増えても、ヘッダーやフッターを毎回書かなくて済むようにする「共通テンプレート」の作り方を学びます。

## c3 「ルーティング」を知る
localhost:8080/aaa/bbb と打った時に、どのコントローラーを動かすか？を設定する config/routes.php を触ってみます。
自分の名前を画面から送信してみる
画面に入力した文字を、コントローラーが受け取って「こんにちは、○○さん！」と表示する「フォーム送信」を体験します。
